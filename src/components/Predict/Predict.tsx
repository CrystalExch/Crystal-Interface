...
const Predict: React.FC<PredictProps> = ({
  ...
  const calculateLiquidation = () => {
    const executionPrice = activeOrderType == 'Market' ? (activeTradeType == 'long' ? activeMarket?.bestAskPrice : activeMarket?.bestBidPrice) : Number(limitPriceString)
    const pendingPositionSize = Number(Math.floor(Number(inputString) / (1 + Number(userFees[0])) / Number(executionPrice) / Number(activeMarket?.stepSize)) * Number(activeMarket?.stepSize));
