...
export default function MemeTradesComponent({
  ...
  }, [trades, market, tradesByMarket, markets, tokendict, usdc, wethticker, ethticker, top10HolderAddresses, currentUserAddress])
  ...
  const renderTraderTags = (tags: ('sniper' | 'dev' | 'kol' | 'bundler' | 'insider')[]) => {
    const tagComponents = {
        ...
        {viewTrades.map(t => (
          <div key={t.id} className="meme-trade-row">
            ...
            <div
              ...
              onClick={() => !t.isCurrentUser && setPopupAddr(t.fullAddress)}
            >
              {t.trader}
            </div>
            <div className="meme-trade-age-container">
              ...
              <span className="meme-trade-age">{fmtTimeAgo(t.timestamp)}</span>
            </div>
          </div>
        ))}
      </div>
